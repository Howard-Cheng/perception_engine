...
typedef void(__cdecl* MeetingCallback)(bool isStart);

typedef void(__cdecl* ErrorCallback)(const wchar_t* model, int errorCode, const wchar_t* msg);
typedef void(__cdecl* progressCallback)(int progressper);

/*
* summary result callback
* @param summary The summarized text.
* @param sessionId The session identifier is from C# side.
* @param success Indicates whether the summarization was successful.
*/
typedef void(__cdecl* SummarizedCallback)(const wchar_t* summary, const wchar_t* sessionId, bool success);
...
extern "C" {
	...
	*/
	...
	DLL_API void UserLoginState(bool state);
}
...
namespace QTSummary {
    ...
    extern "C" {
        ...
        DLL_API wchar_t* StartSummary(const wchar_t* sessionId);
		DLL_API void SummaryOnResult(SummarizedCallback callback);
        DLL_API void SummaryProgress(progressCallback callback);
    }
}
...
namespace MTDetector {

    extern "C" {
        DLL_API bool GetMeetingDetectorResult(MeetingCallback callback);
    }
}

namespace MSFTException {

    extern "C" {
        DLL_API void NoticeCoreError(ErrorCallback callback);
    }
}
namespace BLOBSYNC {
    ...
    extern "C" {
        ...
        DLL_API void IsSavePdfAndJsonFile(const wchar_t* content, const wchar_t* filename);
        DLL_API void deleteBlobSyncByFileName(const wchar_t* fileName);
        DLL_API void startBlobSync(const wchar_t* jsonDirPath);
        DLL_API void stopBlobSync();
        DLL_API wchar_t* getContentByFileName(const wchar_t* filename);
    }
}
